...
AD Updates/comments
 ...
 -- Added the actual trigger locations. There are 12. Find each by searching for comment TRIGGER1 ... TRIGGER2, etc...
    ...
    --. ISI onset (this was initially "12", but it's actually identical to 1.)
...
class WordsInNoiseEEG(object):
    ...
    def netstation_sendtrigger(self,label,timestamp): # ,table):
        if self.UseNetStation:
            self.ns.send_event( 'evt_', label=label, timestamp=timestamp)
    ...
    def intertrial_interval(self, trial_volume, trial_params):
        ...
        # Send TRIGGER1 at the exact moment .flip() is called - the onset of ISI
        self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'prestim_period_onset',timestamp=self.ms_localtime) #adtest
    ...
    def difficulty_cue_period(self,trial_params): # added ad 2/8/17
       # start precise timer
        ...
        self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'diff_cue_onset',timestamp=self.ms_localtime) #adtest
    ...
    def poststimulus_waiting_period(self):
        ...
        # TRIGGER9 - at the exact moment .flip() is called - wait cue onset.
        self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'wait_cue_onset',timestamp=self.ms_localtime) #adtest
        self.EXP_WINDOW.flip()
    ...
    def answer_choice_period(self):
        ...
        # TRIGGER10 - at the exact moment .flip() is called - answer choice onset
        self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'answer_choice_onset',timestamp=self.ms_localtime) #adtest
        self.EXP_WINDOW.flip()
    ...
    def play_stimulus(self):
        ...
        if self.CONDITION == "Visual":
            ...
            # TRIGGER4-B (video trials) stim presentation window onset
            self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'stim_presentation_window_onset',timestamp=self.ms_localtime) #adtest
            ...
            # TRIGGER7-B (video trials) video ended.
            self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'video_ended',timestamp=self.ms_localtime) #adtest
            ...
            # TRIGGER4-A (non-video trials) stim presentation window onset
            self.EXP_WINDOW.callOnFlip(self.netstation_sendtrigger,'stim_presentation_window_onset',timestamp=self.ms_localtime) #adtest
            self.EXP_WINDOW.flip()
